...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Pirate Weather');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!apiKey) throw new Error('Pirate Weather requires an API key: pass your key as the _apiKey argument (free at https://pirate-weather.apiable.io).');
  ...
  if (!res.ok) throw await httpError(res, 'Pirate Weather');
  return res.json();
}
